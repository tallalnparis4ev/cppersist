...
void runFib(vector<int>& input, string type, bool cppersist, bool recursive, bool keepCache){
  string path = getOutPath("Fib",type,cppersist,recursive,keepCache);
  if(recursive){
    FibRec rec;
    auto localMemo = 
      getLocalMemoizedObj<FibRec>(fibBIKey,fibBIPickle,fibBIUnpickle,"fibTest",fibBIHash);
    if(!cppersist){
    ...
    FibIter iter;
    auto localMemo = 
      getLocalMemoizedObj<FibIter>(fibBIKey,fibBIPickle,fibBIUnpickle,"fibTest",fibBIHash);
    if(!cppersist){
...
void runFibTestSeq(vector<int>& input, bool cppersist, bool recursive, bool keepCache){
  runFib(input,"Seq",cppersist,recursive,keepCache);
}

void runFibTestWORep(vector<int>& input, bool cppersist, bool recursive, bool keepCache, int seed){
  shuffle(input.begin(), input.end(), default_random_engine(seed));
  runFib(input,"WORep",cppersist,recursive,keepCache);
}

void runFibTestWRep(vector<int>& input, bool cppersist, bool recursive, bool keepCache, int seed){
  srand(seed);
  vector<int> newInp;
