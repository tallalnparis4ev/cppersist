...
TEST(MongoDBCacheTests, WorkingCorrectly) {
  ...
  EXPECT_EQ(test.get(100).value_or(2), 1) //make sure key 100 returns the value 1
      << "===================================================" << std::endl
      << "The reason for this failure is most likely because" << std::endl
      << "the MongoDB server is not running on localhost:5000" << std::endl
      << "===================================================" << std::endl;
  ASSERT_EQ(test.get(101), nullopt); //make sure key 101 returns the value nullopt - it's not in the cache
}