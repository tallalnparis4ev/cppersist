...
TEST(Fibonacci, TestMongoCache) {  // mongo
  auto fibSolver = getMongoMemoizedObj<FibonacciSolver>(
      keymaker, intostr, strtoi, "localhost:5000", "test");
...
TEST(Fibonacci, FineInputMem) {  // in-memory
  auto fibSolver = getMemoizedObj<FibonacciSolver>(keymaker, intostr, strtoi);
  ASSERT_EQ(fibSolver(8), 13);  // ensure correct answer
}
