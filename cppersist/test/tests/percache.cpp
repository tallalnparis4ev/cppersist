...
TEST(MongoDBCacheTests, WorkingCorrectly){
  MongoDBCache<int,int> test(&intSimple::pickle,&intSimple::pickle,&intSimple::unpickle,&sha256,"localhost:5000");
  test.put(1,1);
  ...
  << std::endl
  << "the MongoDB server is not running on localhost:5000"
  << std::endl
  << "==================================================="
  << std::endl;
  ASSERT_EQ(test.get(2),nullopt);
